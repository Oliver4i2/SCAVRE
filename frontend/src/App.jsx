        ...
        {showSplash && (
          <div className="splash-screen">
            <div className="splash-circle">
              <span className="splash-text">SCAVRE</span>
            </div>
          </div>
        )}
          ...
          {!loginPortal ? (
            <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center', maxWidth: '800px' }} className="animate-fade">
              {[
                ...
                { id: 'admin', icone: '⚙️', texto: 'Administrador TI', cor: '#475569' }
              ].map(p => (
                <button 
                  key={p.id} 
        ...
        <div style={{ padding: '40px', maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
          ...
          {userRole === 'operador' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }} className="animate-fade">
              ...
              <section className="card-glass" style={{ borderTopColor: '#475569', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
